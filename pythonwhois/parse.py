...
grammar = {
    "_data": {
        ...
        'registrar': [r'registrar:\s*(?P<val>.+)',
                      ...
                      r'\tName:\t\s(?P<val>.+)'],
        'whois_server': [r'Registrar WHOIS Server:\s?(?P<val>.+)',
...
registrant_regexes = [
    ...
    # .au educational, company, 'incorporated association' (non-profit?),
    # other (spotted for linux.conf.au, unsure if also for others)
...
nic_contact_references = {
    "registrant": [
        ...
        r"holder:\s*(?P<handle>.+)",
        # iis.se (they apparently want to be difficult, and won't give you contact info for the
        # handle over their WHOIS service)
    ],
    "tech": [
...
def parse_raw_whois(raw_data, normalized=None, never_query_handles=True, handle_server=""):
    ...
    for segment in raw_data:
        ...
        match = re.search(r"    Registration status:\n        (.+)\n", segment)
        if match is not None:
        ...
        match = re.search(r"Registered By:\n\t(.+)\n", segment)
        if match is not None:
            data["registrar"] = [match.group(1).strip()]
        match = re.search(r"Entry created:\n\t(.+)\n", segment)
        if match is not None:
            data["creation_date"] = [match.group(1).strip()]
        match = re.search(r"Renewal date:\n\t(.+)\n", segment)
        if match is not None:
            data["expiration_date"] = [match.group(1).strip()]
        match = re.search(r"Entry updated:\n\t(.+)\n", segment)
        if match is not None:
...
def normalize_data(data, normalized):
    for key in ("nameservers", "emails", "whois_server"):
        if key in data and data[key] is not None and (normalized or key in normalized):
    ...
    for contact_type, contact in data['contacts'].items():
        if contact is not None:
            ...
            for key in ("email",):
                if key in contact and contact[key] is not None and (normalized or key in normalized):
...
def normalize_name(value, abbreviation_threshold=4, length_threshold=8, lowercase_domains=True, ignore_nic=False):
    ...
    for line in value.split("\n"):
        ...
        if (line.isupper() or line.islower()) and len(line) >= length_threshold:
            ...
            if ignore_nic and "nic" in line.lower():
        ...
        for regex in nic_contact_references[category]:
            for segment in data:
                ...
                if match is not None:
                    ...
                    if data_reference["handle"] == "-" or \
                            re.match(r"https?:\/\/", data_reference["handle"]) is not None:
                        # Reference was either blank or a URL; the latter is to deal with false positives for nic.ru
                        pass
                    else:
                        found = False
    ...
    for obj in (registrant, tech_contact, billing_contact, admin_contact):
        if obj is not None:
            ...
            if 'street' in obj and "\n" in obj["street"] and 'postalcode' not in obj:
                # Deal with certain mad WHOIS servers that don't properly delimit address
                # data... (yes, AFNIC, looking at you)
            ...
            if 'country' in obj and 'city' in obj and \
                    (re.match(r"^R\.?O\.?C\.?$", obj["country"], re.IGNORECASE) or
                     obj["country"].lower() == "republic of china") and obj["city"].lower() == "taiwan":
                # There's an edge case where some registrants append ",
                # Republic of China" after "Taiwan", and this is mis-parsed
