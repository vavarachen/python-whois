...
def get_whois_raw(domain, server="", previous=None, rfc3490=True, never_cut=False, with_server_list=False,
                  server_list=None, timeout=TIMEOUT):
    ...
    exceptions = {
        ...
        # The following is a bit hacky, but IANA won't return the right answer for example.com
        # because it's a direct registration.
    ...
    if never_cut:
        ...
        # will be missing header, footer, and alternative domain options (this is handled a few lines below, after the
        # verisign-grs processing).
        ...
        if match is not None:
            ...
            if referal_server != server and "://" not in referal_server:
                ...
                return get_whois_raw(domain, referal_server, new_list, server_list=server_list,
                                     with_server_list=with_server_list, timeout=timeout)
    if with_server_list:
...
def get_root_server(domain, timeout=(5, 10)):
...
def whois_request(domain, server, port=43, timeout=(5, 10)):
